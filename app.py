...
def generate_accounts_flow():
    ...
    while True:
        try:
            account_count = int(input(f"\n{get_random_color()}{Colors.BRIGHT}🎯 How many accounts to generate: {Colors.RESET}"))
            if account_count > 0:
                break
            else:
    ...
    while True:
        try:
            rarity_threshold = int(input(f"\n{get_random_color()}{Colors.BRIGHT}⭐ Enter rarity score threshold (1-10, Recommended: 3): {Colors.RESET}"))
    ...
    while True:
        try:
            thread_count = int(input(f"\n{get_random_color()}{Colors.BRIGHT}🧵 Enter thread count (Recommended: {recommended_threads}): {Colors.RESET}"))
            if thread_count > 0:
                break
            else:
